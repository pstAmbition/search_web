...
def init_args():
    ...
    argument.add_argument('--cache_dir', type=str, help='pretrained model cache directory', default='./bert-base-chinese')
    ...
    argument.add_argument('--best_model', type=str, default='./saved_model-tagger-context/bert-base-chinese_10.pth')
    ...
    argument.add_argument('--dataset_name', type=str, default='weibo2')
    argument.add_argument('--train', type=str, default='False')
    argument.add_argument('--task_mode',type=str,default="inference")
    argument.add_argument('--text',type=str,default='')
    ...
    argument.add_argument('--dropout_rate', type=float, default=0.5)
    ...
    argument.add_argument('--adam_eps', type=float, default=1e-07)
    ...
    argument.add_argument('--char_window_size', type=int, default=5)
    ...
    argument.add_argument('--model_name', type=str, default='./bert-base-chinese')
...
def pretrained_mode(args):
    ...
    test_source = os.path.join(args.dataset_dir, args.task_type, args.dataset_name, 'valid.txt')
    ...
    model = BertForSeqTask.from_pretrained(args.model_name, config=config_,ignore_mismatched_sizes=True)
    ...
    if args.task_mode == "inference":
        print("inference",device)
        return NER(best_model_path,tokenizer_,args,args.text,device,idx2label)
    else:
        return test(best_model_path, args, test_loader, device, metric(idx2label))
...
def train(model,
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        eval_f1, fine_grained_f1,auroc,acc = evaluate(eval_loader, metric_, model, args.device,
        ...
        logger.info("precision {:.4f} recall {:.4f} f1 {:.4f} acc{:.4f} auroc{:.4f}".format(eval_f1['precision'], eval_f1['recall'],
                                                                      eval_f1['f1'],acc,auroc))
...
def NER(best_model_path,tokenizer, args, text, device, idx2label):
    current_gpu = torch.cuda.current_device()
    
    # print("device",torch.cuda.current_device())
    model_test = torch.load(best_model_path,map_location=torch.device(f'cuda:{current_gpu}'))
    # print("device",torch.cuda.device)
    model_test.to(device)
    # print(model_test.device)
   
    # 修复：中文文本按字符分割，而不是按空格分割
    # input_token = tokenizer([text.split()], padding='longest', is_split_into_words=True,
    #                                     truncation=True, return_tensors='pt').to(device)
    input_token = tokenizer([list(text)], padding='longest', is_split_into_words=True,
                                        truncation=True, return_tensors='pt').to(device)
    
    # 添加调试信息
    print(f"输入文本: {text}")
    print(f"字符分割: {list(text)}")
    print(f"输入token长度: {len(input_token['input_ids'][0])}")
    
    # print(input_token)
    outputs = model_test(**input_token)
    probabilities = F.softmax(outputs[1][0],dim=1)[0]
    # print(probabilities.tolist())
    probabilities_list = probabilities.tolist()
    
    logits = torch.argmax(outputs[1][0], dim=-1).int()[0]
    # print(logits.tolist())
    pred_labels = [idx2label[pred_idx] for pred_idx in logits.tolist()]
    print(f"预测标签: {pred_labels}")
    # print(pred_labels)
    scores = [probabilities_list[index][pred_idx] for index,pred_idx in enumerate(logits.tolist())]
    # print(scores)
...
def test(best_model_path, args, test_loader, device, metric):
    if best_model_path:
        ...
        f1, fined_f1,auroc,acc = evaluate(test_loader, metric, model_test,
                                device, args.mode, eval(args.use_crf))
        f.write(best_model_path + '\n')
        f.write("Overall results: \n")
        logger.info("Overall results: ")
        logger.info("precision {:.4f} recall {:.4f} f1 {:.4f} acc {:.4f} auroc {:.4f} \n".format(f1['precision'], f1['recall'],
                                                                      f1['f1'],acc,auroc))
        f.write("precision {:.4f} recall {:.4f} f1 {:.4f} acc {:.4f} auroc {:.4f} \n".format(f1['precision'], f1['recall'],
                                                                     f1['f1'],acc,auroc))
        logger.info("fine_grained_f1: ")
        f.write("fine_grained_f1: \n")
        result = "Overall results: "+"precision {:.4f} recall {:.4f} f1 {:.4f} acc {:.4f} auroc {:.4f} \n".format(f1['precision'], f1['recall'],
                                                                     f1['f1'],acc,auroc)+' ; '
        for key, fined_f1 in fined_f1.items():
            ...
            f.write("{}: precision {:.4f} recall {:.4f} f1 {:.4f}\n".format(key,
                                                                            ...
                                                                            fined_f1['recall'],
                                                                            fined_f1['f1']))
            result += " ; {}: precision {:.4f} recall {:.4f} f1 {:.4f}".format(key,
                                                                              fined_f1['precision'],
                                                                              fined_f1['recall'],
                                                                              fined_f1['f1'])
...
def evaluate(data_loader, metrics, model, device, mode='pretrained', use_crf=False):
    ...
    for batch in data_loader:
        ...
        if mode == 'pretrained':
            if use_crf:
                ...
                logits = torch.argmax(output[1], dim=-1).float()
                
                gold_label=batch['labels']
                probabilities = torch.softmax(output[1], dim=-1)
                # print(probabilities.shape)
                probabilities_np = probabilities.cpu().detach().numpy()
                labels_np = gold_label.cpu().detach().numpy()
                pred_labels = logits.flatten().cpu().tolist()
                # 获取类别数量
                ...
                for class_label in range(num_classes):
                    ...
                    binary_labels = (labels_np == class_label).astype(int)
                    class_probs = probabilities_np[:, :, class_label]
                    all_binary_labels.append(binary_labels.flatten())
                    all_class_probs.append(class_probs.flatten())

                    
                
                gold_label_list = batch['labels'].flatten().cpu().tolist()
                # print(gold_label_list)
                sample_num += len(gold_label_list)
                for index,value in enumerate(gold_label_list):
                    if value == -100:
                        sample_num -= 1
                        
                        continue
                    elif value == pred_labels[index]:
                        acc_count += 1
    ...
    all_class_probs = np.concatenate(all_class_probs)
    average_auroc = roc_auc_score(all_binary_labels, all_class_probs)
    f1, fine_grained_f1 = metrics.calculate_f1()
    return f1, fine_grained_f1,average_auroc,acc_count/sample_num
...
def process_text(text):
    ...
    return pretrained_mode(arguments)
