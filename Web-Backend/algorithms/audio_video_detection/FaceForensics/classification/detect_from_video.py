...
def test_full_image_network_2(video_fake_path, video_real_path, model, output_path,
    ...
    confidence = np.mean(probabilities_list)
...
def load_model():
    model_path = "FaceForensics/classification/models/models_subset/full/xception/full_raw.p"
    model = torch.load(model_path)

    return model

def video_classification_main_function_2(model, url):
    video_fake_path = "altered"
    video_real_path = "original"
    ...
    video_fake_path = os.path.join(url, video_fake_path)
    video_real_path = os.path.join(url, video_real_path)

    pre_list = []
    true_list = []
    probabilities_list = []
    if video_fake_path.endswith('.mp4') or video_fake_path.endswith('.avi'):
        pre_list, true_list = test_full_image_network(video_fake_path,video_real_path,model,output_path,start_frame,end_frame,cuda,is_fake)
    else:
        fake_videos = os.listdir(video_fake_path)
        videos_fake_10 = fake_videos[:2] #抽样
        for video in tqdm(videos_fake_10):
            video_fake_path = join(video_fake_path, video)
            tqdm.write('Starting: {}'.format(video_fake_path))
            pre, tr, pro = test_full_image_network(video_fake_path,video_real_path,model,output_path,start_frame,end_frame,cuda,is_fake)
        ...
        videos_real_10 = real_videos[:2] #抽样
        for video in tqdm(videos_real_10):
            video_real_path = join(video_real_path, video)
            tqdm.write('Starting: {}'.format(video_real_path))
            is_fake = False
            pre, tr, pro = test_full_image_network(video_fake_path,video_real_path,model,output_path,start_frame,end_frame,cuda,is_fake)
            pre_list = pre_list + pre
            true_list = true_list + tr
...
def test_full_image_network(video_path, model, output_path,
    ...
    if len(probabilities_list) > 0:
        ...
        confidence = 0.5  # 如果没有检测到人脸，使用默认值
    ...
    return {"labels": category, "scores": float(confidence)}
...
def video_classification_main_function(model, url):
    ...
    cuda = True
    is_fake = True
    result = test_full_image_network(url, model,output_path,start_frame,end_frame,cuda,is_fake)
...
if __name__ == '__main__':
    ...
    model = torch.load(model_path)
    video_classification_main_function_2(model = model, url = url)
