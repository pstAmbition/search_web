...
def execute_custom_query():
    ...
    if not nebula_service or not hasattr(nebula_service, 'execute_query'):
        return jsonify({"error": "Nebula服务未可用"}), 503

    try:
        space_name = current_app.config.get('NEBULA_SPACE', 'default')
        ...
        return jsonify({"error": f"Nebula查询执行失败: {str(e)}"}), 500
...
def search_text_route():
    ...
    if not search_service:
        return jsonify({"error": "搜索服务未可用"}), 503
    ...
    try:
        ...
        return jsonify({"error": f"文本搜索失败: {str(e)}"}), 500
...
def search_picture_route():
    ...
    try:
        ...
        return jsonify({"error": f"图片搜索失败: {str(e)}"}), 500
...
def search_video_route():
    ...
    try:
        ...
        return jsonify({"error": f"视频搜索失败: {str(e)}"}), 500
...
def get_all_fake_knowledge_handler():
    ...
    try:
        ...
        logger.info(f"成功获取所有虚假信息，共 {len(result_info)} 条")
        return jsonify({
            "success": True,
            "data": result_info
        })
...
def search_fake_knowledge_handler():
    ...
    try:
        ...
        logger.info(f"搜索虚假信息成功，关键词: {keyword}，找到 {len(search_results)} 条结果")
        return jsonify({
            "success": True,
            "data": search_results
        })
    except Exception as e:
        logger.error(f"搜索虚假信息失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "data": []
...
def get_fake_media_handler(fake_id):
    ...
    try:
        ...
        if os.path.exists(img_dir):
            ...
            for filename in os.listdir(img_dir):
                if filename.startswith(fake_id) or filename == fake_id:
                    ...
                    logger.info(f"找到本地图片文件: {filename}")
        ...
        if os.path.exists(video_dir):
            ...
            for filename in os.listdir(video_dir):
                if filename.startswith(fake_id) or filename == fake_id:
                    ...
                    logger.info(f"找到本地视频文件: {filename}")
        ...
        if not media["pictures"] and not media["videos"]:
            if neo4j_service:
                try:
                    ...
                    logger.warning(f"从Neo4j获取媒体资源失败: {str(e)}, fake_id: {fake_id}")
            ...
            if os.path.exists(os.path.join(uploads_dir, filename)):
                ...
                if os.path.exists(os.path.join(img_dir, filename)):
                    ...
                    try:
                        ...
                        valid_pictures.append(img_url)
                        logger.info(f"已复制图片文件到uploads目录: {filename}")
                    except Exception as e:
                        logger.error(f"复制图片文件失败: {str(e)}")
            ...
            if os.path.exists(os.path.join(uploads_dir, filename)):
                ...
                if os.path.exists(os.path.join(video_dir, filename)):
                    ...
                    try:
                        ...
                        valid_videos.append(video_url)
                        logger.info(f"已复制视频文件到uploads目录: {filename}")
                    except Exception as e:
        ...
        logger.info(f"返回媒体资源: 图片{len(media['pictures'])}个, 视频{len(media['videos'])}个")
        ...
        logger.error(f"获取虚假信息多媒体资源时发生错误: {str(e)}, fake_id: {fake_id}")
        return jsonify({
            "success": True,
            "data": {
                "pictures": [],
